...
namespace bq25820
{
    ...
    bool Bq25820::initialize(std::string &device_name)
    {
    ...
    bool Bq25820::spin_ADC()
    {
        ...
        if ((elap_since_ADC_read_ > elap_since_ADC_read_interval_) && adc_OneShot_in_progress_)
            {
                // RCLCPP_INFO(rclcpp::get_logger("BQ25820"), "Reading ADC values...");
                adc_OneShot_in_progress_ = false;
                read_all_ADC_values();
                elap_since_ADC_read_ = 0;
                has_changes          = update_statuses();
                if (elap_since_last_ADC_display_ > elap_since_last_ADC_display_interval_)
                    {
                        elap_since_last_ADC_display_ = 0;
                        display_ADC_values();
                    }
            }
        return has_changes;
    ...
    bool Bq25820::spin_status_change()
    {
        bool has_changes = false;
    ...
    bool Bq25820::spinOnce()
    {
        ...
        if (elap_since_Status_display_ > elap_since_Status_display_interval_)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("BQ25820"), status.c_str());
            }
        //~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+
        if (!is_charging_)
            {
                if (should_start_charging())
                    {
                        ...
                        if (beginCharging())
                            {
                                is_charging_ = true;
                                RCLCPP_INFO(rclcpp::get_logger("BQ25820"), "Charging started...!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                            }
        ...
        //~`=-+
        //~`=-+  ADC handling
        something_changed |= spin_ADC();

        //~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+~`=-+
        //~`=-+
        //~`=-+  Status handling
        something_changed |= spin_status_change();
    ...
    void Bq25820::check_gpio_changes()
    {
        ...
        if (gpio_STAT1_current != gpio_STAT1_previous_)
            {
                RCLCPP_INFO(rclcpp::get_logger("BQ25820"), "GPIO STAT1 changed from %d to %d", gpio_STAT1_previous_, gpio_STAT1_current);
            }
        if (gpio_STAT2_current != gpio_STAT2_previous_)
            {
                RCLCPP_INFO(rclcpp::get_logger("BQ25820"), "GPIO STAT2 changed from %d to %d", gpio_STAT2_previous_, gpio_STAT2_current);
            }
        if (gpio_PG_current != gpio_PG_previous_)
            {
                RCLCPP_INFO(rclcpp::get_logger("BQ25820"), "GPIO PG changed from %d to %d", gpio_PG_previous_, gpio_PG_current);
            }
        if (gpio_CE_current != gpio_CE_previous_)
            {
                RCLCPP_INFO(rclcpp::get_logger("BQ25820"), "GPIO CE changed from %d to %d", gpio_CE_previous_, gpio_CE_current);
            }
        ...
        gpio_STAT1_previous_ = gpio_STAT1_current;
        gpio_STAT2_previous_ = gpio_STAT2_current;
        gpio_PG_previous_    = gpio_PG_current;
    ...
    float Bq25820::get_battery_voltage() const { return ADC_VBAT_value_volts_; }
    ...
    float Bq25820::get_battery_percentage() const
    {
        float V = get_battery_voltage();
        if (V > max_volts_4S) V = max_volts_4S;
        if (V < min_volts_4S) V = min_volts_4S;
        float V_base                   = V - min_volts_4S;
        float V_span                   = max_volts_4S - min_volts_4S;
        float percent                  = (V_base / V_span) * 100.0F;
        int   battery_percentage       = static_cast<int>(percent * 10.0F);
        float battery_percentage_round = static_cast<float>(battery_percentage) / 10.0F;
        return battery_percentage_round;
    }
    ...
    void Bq25820::display_ADC_values()
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("BQ25820"), values.c_str());
    }
    ...
    bool Bq25820::check_for_charging_status_change()
    {
    ...
    {
        std::string status;
        uint8_t     changes = get_status_bits_changes(flag2_previous_, flag2_current_);
    ...
    std::string Bq25820::get_BQ25820_Charge_Status_str(BQ25820_Charge_Status status)
    {
        switch (status)
            {
                case BQ25820_Charge_Status::Not_charging: return "Not charging";
                case BQ25820_Charge_Status::Trickle_Charge: return "Trickle Charge";
                case BQ25820_Charge_Status::Pre_Charge: return "Pre-Charge";
                case BQ25820_Charge_Status::Fast_Charge: return "Fast Charge";
                case BQ25820_Charge_Status::Taper_Charge: return "Taper Charge";
                case BQ25820_Charge_Status::Reserved_0x05: return "Reserved (0x05)";
                case BQ25820_Charge_Status::Top_off_Timer_Charge: return "Top-off / Timer Charge";
                case BQ25820_Charge_Status::Charge_Termination_Done: return "Charge Termination Done";

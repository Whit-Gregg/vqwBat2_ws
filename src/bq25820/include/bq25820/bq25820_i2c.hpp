    ...
    class BQ25820_I2C
    {
      private:
        ...
        bool read_regs(uint8_t reg, uint8_t *buf, size_t len) const
        {
            ...
            if (rc < 0)
                {
                    RCLCPP_ERROR(rclcpp::get_logger("BQ25820"), "Failed to read I2C registers, ioctl rc=%d  errno=%d", rc, errno);
                    return false;
                }
        ...
        bool write_regs(uint8_t reg, uint8_t *buf, size_t len) const
        {
            ...
            if (rc < 0)
                {
                    RCLCPP_ERROR(rclcpp::get_logger("BQ25820"), "Failed to write I2C registers, ioctl rc=%d  errno=%d", rc, errno);
                    return false;
                }
      ...
      public:
        ...
        uint8_t read_reg8(BQ25820_Register reg) const { return read_reg8(static_cast<uint8_t>(reg)); }
        ...
        uint16_t read_reg16(BQ25820_Register reg) const { return read_reg16(static_cast<uint8_t>(reg)); }
